...
describe('deck presets', () => {
  ...
  it('starts every preset with the ? card', () => {
    expect(DECK_PRESETS.every((d) => d.cards[0] === QUESTION_CARD)).toBe(true)
  })
})
...
describe('normalizeCustomDeck', () => {
  ...
  it('moves a user-typed ? to the front', () => {
  ...
  it('truncates over-long card values', () => {
    expect(normalizeCustomDeck('verylongcardvalue')[1]).toHaveLength(8)
  })
})
...
describe('normalizeStoredDeck', () => {
  ...
  it('drops the last stored card when a full deck needs room for the ?', () => {
    ...
    expect(normalizeStoredDeck(full)).toEqual([QUESTION_CARD, ...full.slice(0, MAX_DECK_CARDS - 1)])
  })
})

describe('withQuestionCard', () => {
  it('prepends ? when missing and keeps it first', () => {
    expect(withQuestionCard(['1', '2'])).toEqual(['?', '1', '2'])
  })

  it('moves an existing ? to the front without duplicating', () => {
  ...
  it('caps at MAX_DECK_CARDS including the ?', () => {
    ...
    expect(deck[0]).toBe(QUESTION_CARD)
  })
})
